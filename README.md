...
In this repo, I'll go through some simple examples of how to create a website with FastAPI.

- **ex0_simple_html**: A simple HTML file that is not served by FastAPI. Test this
  on Vercel. No special setup required.
- **ex1_hello**: A simple "Hello, World!" page served by FastAPI. This one will not
  work on Vercel as-is. This example just demonstrates how to set up a simple,
  traditional server.
- **ex2_templates**: A simple page with a Jinja2 template served by FastAPI. This
  example also won't work on Vercel as-is. It demonstrates how to use Jinja2
  templates to serve a simple page.
- **ex3_static_files**: A simple page with static files served by FastAPI. This
  example WILL work on Vercel. This example just simply mounts the static HTML
  files in the `main` directory.
- **ex4_sphinx**: A simple page with Sphinx documentation served by FastAPI. This
  example will work on Vercel. It is just a more realistic example where I mount
  the directory of HTML files generated by Sphinx.
- **ex5_simple_auth**: A simple page with authentication served by FastAPI. This
  example will also work on Vercel. This is the simplest way to add
  authentication to a website. It uses the Clerk Python SDK to add
  authentication to the files served by FastAPI.
